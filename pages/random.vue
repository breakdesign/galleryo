<template>
  <div class="container">
    <div class="wrapper">
      ...
      <div class="bd-c-random">
        <div class="bd-c-content">
          <ul id="grid" class="bd-c-random__list grid effect-2">
            <li
              v-for="(item, index) in dataFromAPI"
              :key="index"
              :class="'bd-c-random__items ' + (index % 3 ? 'h196' : 'h259')"
            >
...
export default {
  ...
  computed: {
    ...mapState({
      dataFromAPI: (state) => state.unsplash.popular
    })
  },
  mounted() {
    this.getCategory()
  ...
  methods: {
    async getCategory() {
      await this.$store.dispatch('unsplash/getDataPopularAll')
    },
    submitSearch() {
